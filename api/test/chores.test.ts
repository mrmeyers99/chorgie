...
describe("POST /chores", () => {
  ...
  it("accepts always-available as a recurrence_type", async () => {
    const mockClient = await getMockClient();
    mockClient.query
      .mockResolvedValueOnce({ rows: [] }) // BEGIN
      .mockResolvedValueOnce({
        rows: [{ ...sampleChore, recurrence_type: "always-available" }],
      }) // INSERT chore
      .mockResolvedValueOnce({ rows: [] }); // COMMIT
  ...
  it("rejects eligible_kids from a different household", async () => {
    ...
    expect(res.body.error).toBe(
      "eligible_kids must reference kids in this household.",
    );
  });
});
...
describe("POST /chores/:id/complete", () => {
  ...
  it("completes an available chore and updates balance", async () => {
    ...
    mockClient.query
      ...
      .mockResolvedValueOnce({
        rows: [{ completed_at: "2026-05-31T00:00:00.000Z" }],
      }) // completion insert
    ...
    expect(mockClient.query.mock.calls[5]?.[0]).toContain(
      "SET next_available_at",
    );
    expect(mockClient.query.mock.calls[5]?.[1]).toContain(
  ...
  it("keeps always-available chores active and available after completion", async () => {
    const kidId = "11111111-1111-4111-8111-111111111111";
    const mockClient = await getMockClient();
    mockClient.query
      .mockResolvedValueOnce({ rows: [] }) // BEGIN
      ...
      .mockResolvedValueOnce({ rows: [{ balance: "5.00" }] }) // balance update
      .mockResolvedValueOnce({ rows: [] }); // COMMIT (no chore_definitions update)
    ...
    expect(res.body.reward_amount).toBe("2.50");
    expect(res.body.balance).toBe("5.00");
    expect(mockClient.query.mock.calls).toHaveLength(6);
...
describe("POST /chores/:id/override-availability", () => {
  ...
  it("rejects override for inactive chores", async () => {
    ...
    expect(res.status).toBe(400);
    expect(res.body.error).toBe(
      "Cannot override availability for inactive chores.",
    );
  });
  ...
  it("rejects override for non-recurring chores", async () => {
    ...
    expect(res.status).toBe(400);
    expect(res.body.error).toBe(
      "Can only override availability for recurring chores.",
    );
  });

  it("requires authentication", async () => {
    const res = await request(app).post(
      "/chores/chore-1/override-availability",
    );
